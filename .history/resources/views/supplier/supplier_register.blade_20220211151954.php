                ...
                <form method="POST" action="supplier_store">
                    ...
                    <div class="mt-4">
                        ...
                        <div class="flex">
                            ...
                            _
                            <x-input id="tel3" class="block mt-1 ml-3 w-1/3 text-xs" type="text" :value="old('tel3')"
                                name="tel3" maxlength="5" placeholder="必須" />
                        </div>
                    ...
                    <div class="mt-4">
                        ...
                        <table class="registerTable text-xs">
                            ...
                            <tr class="tr">
                                ...
                                <td id="selectBox" class="td bg-white">
                                    <select id="plantList"
                                        ...
                                        type="text" name="plantId">
                                        <option value="" class="hidden" disabled selected>選択してください</option>
                                        @foreach ($plants as $plant)
                                            <option value="{{ $plant->plant_id }}"
                                                @if (old('plantList') === '{{ $plant->plant_id }}')
                                                    selected
                                                @endif
                                            >
                                            {{ old($plant->plant_name) }}（{{ old($plant->plant_id) }}）
                                            </option>
                                            <input type="hidden" value="{{ old($plant->plant_name) }}">
                                            <input type="hidden" value="{{ old($plant->plant_id) }}">
                                        @endforeach
                                    </select>
                                    <span type="text" id="modalSelected" class="pl-4 border-none inline-block w-full text-xs"></span>
                                    <input type="hidden" id="selectedId" name="shopId" value="">
                                </td>
                            </tr>
                        </table>
        ...
        <div class="selectModalWrapper">
            <div class="modalContent w-auto">
                ...
                <div class="border border-black mt-8">
                    <div class="text-center border-b border-black py-1 px-3 text-xs selectModalTitle">店舗（ID）</div>
                    <div id="selectModalList">
                        <ul>
                            @foreach ($shops as $shop)
                                <li class="py-1 px-3 text-xs border-b hover:bg-gray-200" data-id="{{ $shop->shop_id }}"><a
                                        href=#>{{ $shop->shop_name }}（{{ $shop->shop_id }}）</a>
                                </li>
                            @endforeach
                        </ul>
                    </div>
