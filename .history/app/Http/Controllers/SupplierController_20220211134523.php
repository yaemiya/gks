...
class SupplierController extends Controller
{
 ...
 // 仕入業者登録処理
    public function store(Request $request)
    {
        ...
        if($request->businessKind === '2' && !isset($request->plantId)) // 種別工場が選択されていて取引工場が選択されていないとき
        {
            $errs = __('validation.plantId');
        }
        if($request->businessKind === '3' && !isset($request->shopId)) // 店舗が選択されていて取引店舗が選択されていないとき
        {
        ...
        if(!isset($address->id))
        {
            Log::error($func.':Address::create() error! auth user_id='.Auth::user()->user_id);
            $message = __('messages.err.dbErr');
            return view('error', compact('message'));
        }
        ...
        {
            $spr = SupplierPlantRelationship::create([
                'supplier_id' => $supplier->id,
                'plant_id' => $request->plantId
            ]);
            if(!isset($spr->id))
            {
                Log::error($func.':SupplierPlantRelationship::create() error! auth user_id='.Auth::user()->user_id);
                $message = __('messages.err.dbErr');
                return view('error', compact('message'));
            }
        }
        ...
        {
            $ssr = SupplierShopRelationship::create([
                'supplier_id' => $supplier->id,
                'shop_id' => $request->shopId
            ]);
            if(!isset($spr->id))
            {
                Log::error($func.':SupplierShopRelationship::create() error! auth user_id='.Auth::user()->user_id);
                $message = __('messages.err.dbErr');
                return view('error', compact('message'));
            }
        }
