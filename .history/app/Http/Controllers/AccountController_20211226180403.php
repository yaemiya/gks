...
class AccountController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        //     {
        //         $telSumMes = '電話番号の合計桁数は10桁か11桁で入力してください';
        //         redirect()->route('register')->with('error', '電話番号の合計桁数は10桁か11桁で入力してください。');
        //     }
        ...
        if(mb_strlen($request->tel1.$request->tel2.$request->tel3) > 11 || mb_strlen($request->tel1.$request->tel2.$request->tel3) < 10)
        {
           return back()->withInput->with('error', '電話番号の合計桁数は10桁か11桁で入力してください。');
        }
        $request->validate([
        ...
        if(mb_strlen($request->tel1.$request->tel2.$request->tel3) > 11 || mb_strlen($request->tel1.$request->tel2.$request->tel3) < 10)
        {
           redirect()->route('create')->with('error', '電話番号の合計桁数は10桁か11桁で入力してください。');
        }
