...
class SupplierController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if ($request->businessKind === '2')
        {
            ...
            if (!isset($spr->id))
            {
                Log::error($func . ':SupplierPlantRelation::create() error! auth user_id=' . Auth::user()->user_id.' business_kind='.$request->businessKind.' business_kind='.$request->businessKind);
                $message = __('messages.err.dbErr');
                DB::rollBack();
                return view('error', compact('message'));
            }
