...
$(function () {
    ...
    $("#selectModalList li").on("click", function () {
        ...
        $("#modalSelected").val(modalSelected);
        $("#closeSelectModal").trigger("click");
    });

    $("#searchrBtn").on("click", function () {
        //半角スペース区切りの文字を配列に格納する
        var searchArr = $("#searchBox").val().trim().s?plit(/\s+/);
        var searchArr = $("#searchBox").val().trim().split(new RegExp('\s+'));
