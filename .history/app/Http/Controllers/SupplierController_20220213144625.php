...
class SupplierController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if (!isset($address->id))
        {
            Log::error($func . ':Address::create() error! auth user_id='.Auth::user()->user_id . ' postal_code='.$request->postalCode1.' postal_code2='.$request->postalCode2.' postal_code2='.$request->postalCode2.' prefecture='.$request->prefecture.' address='.$request->address.' house_num='.$request->house_num.' building='.$request->building.' tel='.$request->tel);
            $message = __('messages.err.dbErr');
            DB::rollBack();
            return view('error', compact('message'));
        }
        ...
        {
            Log::error($func . ':Supplier::create() error! auth user_id='.Auth::user()->user_id.' supplier_name='.$request->supplier_name.' building='.$request->building.' building='.$request->building.' building='.$request->building);
            $message = __('messages.err.dbErr');
            DB::rollBack();
            return view('error', compact('message'));
        }
        ...
        if ($request->businessKind === '2')
        {
            ...
            if (!isset($spr->id))
            {
                Log::error($func . ':SupplierPlantRelation::create() error! auth user_id=' . Auth::user()->user_id);
                $message = __('messages.err.dbErr');
                DB::rollBack();
                return view('error', compact('message'));
            }
        ...
        elseif ($request->businessKind === '3')
        {
            ...
            if (!isset($ssr->id))
            {
                Log::error($func . ':SupplierShopRelation::create() error! auth user_id=' . Auth::user()->user_id);
                $message = __('messages.err.dbErr');
                DB::rollBack();
                return view('error', compact('message'));
            }
