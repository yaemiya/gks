    ...
    <div class="formContainer">
        <div class="formMain">
            ...
            <div class="flex place-content-end space-x-3">
                @if(Auth::user()->role === '1')
                <button id="editBtn" class="btn btn-green" data-user_id="{{ (Auth::user()->role === '1' }}">編集</button>
                <button id="openModal" class="btn btn-red" data-user_id="{{ $user->user_id }}">削除</button>
                @else
                <button id="editBtn" class="btn btn-disabled notAllowed" data-user_id="{{ $user->user_id }}">編集</button>
                <button id="openModal" class="btn btn-disabled notAllowed" data-user_id="{{ $user->user_id }}">削除</button>
                @endif
            </div>
            <table width="800">
                <tr>
