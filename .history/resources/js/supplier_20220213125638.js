$(function () {
    ...
    if ($("#businessKind").val() === "2") {
         // 取引工場／店舗の種別が工場のとき、プルダウンメニューを表示
        $("#modalSelected").hide();
        $("#plantList").show();
    }

    if ($("#businessKind").val() === "3") // 取引工場／店舗の種別が店舗のとき、プルダウンメニューを非表示
    {
        // $("#plantList").hide();
        $("#modalSelected").show();
    }
    ...
    $("#createBtn").on("click", function () {
        if ($(this).data("role") === 1) {
            window.location.href = "/supplier/supplier_create";
        }
    });
    ...
    $("#validationBtn").on("click", function () {
        // 既存エラーメッセージの消去
        $(
            "#supplierNameErr, #repNameErr, #postalCodeErr, #prefectureErr, #addressErr, #addressErr, #houseNumErr, #buildingErr, #emailErr, #telErr, #businessKindErr"
        ).text("");
        ...
        if ($("#supplierName").val() === "") {
            ...
            $("#supplierNameErr").text(
                "会社名／屋号は30字以内で入力してください。"
            );
            errFlg = 1;
        }
        ...
        if ($("#postalCode1, #postalCode2").val() === "") {
            ...
            !isNumber($("#postalCode1").val()) ||
            !isNumber($("#postalCode2").val())
        ) {
            $("#postalCodeErr").text("郵便番号は半角数字で入力してください。");
            errFlg = 1;
        } else if ($("#postalCode1").val().length !== 3) {
            $("#postalCodeErr").text("郵便区番号は3桁で入力してください。");
            errFlg = 1;
        } else if ($("#postalCode2").val().length !== 4) {
            $("#postalCodeErr").text("町域番号は4桁で入力してください。");
            errFlg = 1;
        }
        ...
        if (
            ...
            ($("#tel1").val() + $("#tel2").val() + $("#tel3").val()).length > 11 ||($("#tel1").val() + $("#tel2").val() + $("#tel3").val()).length < 10)
            console.log(("#tel1").val() + $("#tel2").val() + $("#tel3").val());
            $("#telErr").text(
                "電話番号の合計桁数は10桁か11桁で入力してください。"
            );
            errFlg = 1;
        }
        ...
        if (
            ...
            $("#businessKindErr").text("取引工場を選択してください。");
        }
        if (
            $("#businessKind").val() === "3" &&
            $("#modalSelected").val() === ""
        ) {
            $("#businessKindErr").text("取引店舗を選択してください。");
        }
        console.log($("#modalSelected").val());
